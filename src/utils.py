...
def load_user_settings():
    ...
    try:
        with open(os.path.join(os.path.dirname(__file__), "..", "user_settings.json"), "r") as file:
...
def load_excel_data():
    ...
    try:
        file_path = os.path.join(os.path.dirname(__file__), "..", "data", "my_operations.xlsx")
        transactions_df = pd.read_excel(file_path)

        required_columns = {"Date", "Category", "Amount"}
        if not required_columns.issubset(transactions_df.columns):
            logging.error(f"Файл Excel не содержит всех необходимых столбцов: {required_columns}")
            return pd.DataFrame()
